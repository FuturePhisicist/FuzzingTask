...
def generate_utf8_files(n, size_min, size_max, output_dir="valid_ascii"):
	...
	for i in range(n):
		...
		file_path = os.path.join(output_dir, f"random_ascii_{i}.txt")

		with open(file_path, "w", encoding="ascii") as f:
			written = 0
			while written < file_size:
				rand_char = chr(random.randint(32, 126))  # ASCII valid characters
